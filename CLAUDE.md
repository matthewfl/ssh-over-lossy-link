...
- Log-assert gotcha that cost a scenario cycle: if you change a debug-log marker /
  status line, its ASSERT in test_ssh_oll.py must be updated in the same edit — and
  once a checker is added, re-verify by grepping BOTH the exact format string in the
  C++ code and the exact needle in the checker.
