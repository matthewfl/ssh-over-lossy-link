...
int run_client(const Args& args) {
  ...
  auto flush_carrier_writes = [&]() {
    ...
    if (carriers.empty() && !unacked_sends.empty()) {
      ...
      { static uint64_t last_spam_ns = 0;
        if (dbg && dbg_rate_allow(last_spam_ns, now_ns(), 1000000000ull))
          fprintf(dbg, "[retransmit-needed t=%llu unacked=%zu reason=write_error_all_dead]\n",
                  (unsigned long long)(now_ns()/1000000ULL), unacked_sends.size());
      }
    }
  };
  ...
  while (running) {
    ...
    // ── Timeout-based retransmit / RS stale-drain ────────────────────────────
    {
      ...
      // same way; this block sits on the main loop, not the 500 ms tick) ─────
      static uint64_t last_cli_dump_ns = 0;
      if (dbg && now_p - last_cli_dump_ns >= 1000000000ULL) {
