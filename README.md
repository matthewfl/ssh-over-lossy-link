...
| **Redundancy pressure** | RS redundancy has been raised above 0.65 (above the auto-adapt baseline of 0.6) due to packet loss. Because each RS group uses exactly `n_carriers` shards (one per carrier), every new carrier directly increases `k` (the data shards per group) and restores effective throughput. Raising parity and adding carriers are complementary responses to the same problem. |
